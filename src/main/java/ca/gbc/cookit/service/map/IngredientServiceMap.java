...
public class IngredientServiceMap implements IngredientService {
    ...
    public void removeIngredientForCurrentUser(Long ingredientId, String recipeCode) {
        ...
        this.userService.removeFromAllIngredientLists(ingredientId);
    }

    @Override
    public Ingredient findById(Long id) {
        Optional<Ingredient> ingredientOpt = this.ingredientRepository.findById(id);
        return ingredientOpt.orElseThrow(() -> new BadRequestRuntimeException(Constants.INGREDIENT_NOT_FOUND));
    }
    ...
    public Ingredient findIngredientByIdForCurrentUser(String recipeCode, Long ingredientId) {
        ...
        return recipe.getIngredients().stream().filter(i -> i.getId().equals(ingredientId)).findAny().orElseThrow(() -> new BadRequestRuntimeException(Constants.INGREDIENT_NOT_FOUND));
    }
